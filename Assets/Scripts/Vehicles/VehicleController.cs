...
namespace Overdrive.Vehicles
{
    ...
    public class VehicleController : MonoBehaviour
    {
        ...
        {
            // Update steering
            steeringSystem.ApplySteering(wheels, steering);

            // Update brakes
            brakeSystem.ApplyBrake(wheels, brake, handbrake, currentSpeed, handbrakeActive);

            // Update engine power
            float engineTorque = engine.CalculateTorque(transmission.CurrentGear, engine.currentRPM, currentSpeed, throttle, transmission.gearRatios, transmission.reverseRatio);

            foreach (var wheel in wheels)
            {
                if (drive == Drive.AWD ||
                    (drive == Drive.FWD && wheel.axel == Axel.Front) ||
                    (drive == Drive.RWD && wheel.axel == Axel.Rear))
                {
            ...
            // Update transmission
            transmission.HandleTransmission(engine.currentRPM, engine.redLine, engine.maxRPM);

            // Update speed
            currentSpeed = rb.velocity.magnitude * 3.6f;
            float maxSpeedLimit = transmission.CurrentGear == -1 ? engine.GetMaxReverseSpeed() : engine.maxSpeed;
            currentSpeed = Mathf.Clamp(currentSpeed, 0f, maxSpeedLimit);
        }
        ...
        {
            // Update gear display
            gearText.text = transmission.CurrentGear switch
            {
                0 => "N",
                -1 => "R",
            ...
            speedText.text = $"{Mathf.Round(currentSpeed)} ";
            uphText.text = speedDisplayUnit == SpeedUnit.KPH ? "KPH" : "MPH";

            // Update RPM display
            rpmText.text = Mathf.Round(engine.currentRPM).ToString();
            rpmSlider.maxValue = engine.maxRPM;
            rpmSlider.value = engine.currentRPM;

            // Update colors based on redline
            Color textColor = engine.currentRPM > engine.redLine ? Color.red : Color.white;
            gearText.color = textColor;
            rpmText.color = textColor;
            rpmSlider.fillRect.GetComponent<Image>().color = textColor;
